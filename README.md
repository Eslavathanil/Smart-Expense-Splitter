...
#### `SettlementCard.tsx`
Handles debt settlement with features:
...
:root {
  --primary: 174 84% 40%;      /* Teal accent */
  --background: 0 0% 100%;     /* White background */
  --foreground: 174 10% 10%;   /* Dark text */
  --destructive: 0 84% 60%;    /* Red for errors */
  /* ... more tokens */
}

.dark {
  --background: 174 10% 5%;    /* Dark background */
  --foreground: 174 5% 95%;    /* Light text */
  /* ... dark mode overrides */
}
```
...
```javascript
{
  ...
  splits: { type: Map, of: Number },  // { "Alice": 50, "Bob": 50 }
  createdAt: { type: Date, default: Date.now }
}
```
...
{
  groupId: { type: ObjectId, ref: 'Group', required: true },
  from: { type: String, required: true },    // Who paid
  to: { type: String, required: true },      // Who received
  amount: { type: Number, required: true },
  createdAt: { type: Date, default: Date.now }
}
```
...
function calculateBalances(expenses, settlements) {
  const balances = {};  // { member: netBalance }
  ...
  expenses.forEach(expense => {
    ...
    // Subtract from each member's balance (they owe)
  ...
  settlements.forEach(settlement => {
    balances[settlement.from] += settlement.amount;  // Paid off debt
    balances[settlement.to] -= settlement.amount;    // Received less
  });
